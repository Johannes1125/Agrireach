...
const defaultCategories: Category[] = [
  { _id: "Farming Tips & Techniques", name: "Farming Tips & Techniques", icon: "🌱" },
...
export default function NewThreadPage() {
  ...
  const [categoryOptions, setCategoryOptions] = useState<Category[]>(defaultCategories)
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const items: Category[] = (json?.data?.items || []).map((c: any) => ({ _id: String(c._id || c.name), name: c.name, icon: c.icon }))
        if (res.ok) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <Select value={formData.category || ""} onValueChange={(value) => handleInputChange("category", value)}>
                    ...
                    <SelectContent>
                      {categoryOptions
                        ...
                        .map((category, idx) => {
                          const id = String(category._id || category.name || idx)
                          return (
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">Add up to 3 images to support your discussion</p>
              </CardHeader>
              <CardContent>
                <ImageUpload
                  type="community"
      ...
      <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to post this discussion? Once published, it will be visible to all community members
